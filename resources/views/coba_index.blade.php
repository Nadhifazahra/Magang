  ...
  <div class="min-h-full">
    <nav class="bg-gray-800">
          ...
          <div class="flex items-center">
            ...
            <div class="hidden md:block">
              <div class="ml-10 flex items-baseline space-x-4">
                ...
                <a href="{{ route('coba.create') }}" class="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white">Pengajuan</a>
                <a href="{{ route('coba.index') }}" class="rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white" aria-current="page">Daftar Pengajuan</a>
              </div>
            </div>
          </div>
          <div class="hidden md:block">
      ...
      <div class="md:hidden" id="mobile-menu">
        <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3">
          <!-- Current: "bg-gray-900 text-white", Default: "text-gray-300 hover:bg-gray-700 hover:text-white" -->
          <a href="{{ route('beranda') }}" class="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-gray-700 hover:text-white">Beranda</a>
          <a href="{{ route('coba.create') }}" class="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white">Pengajuan</a>
          <a href="{{ route('coba.index') }}" class="rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white" aria-current="page">Daftar Pengajuan</a>
        </div>
    ...
    <header class="bg-white shadow">
      <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <h1 class="text-3xl font-bold tracking-tight text-gray-900">Daftar Calon Peserta Magang</h1>
      </div>
    </header>
    <main>
      
      <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      @foreach($dataCoba as $coba)
        <ul role="list" class="divide-y divide-gray-100">
          <li class="flex items-center justify-between gap-x-6 py-5">
            <div class="flex min-w-0 gap-x-4">
              @if($coba->jenis_kelamin === 'Laki-laki')
              ...
              <div class="min-w-0 flex-auto">
                <p class="text-sm font-semibold leading-6 text-gray-900">{{ $coba->nama }}</p>
              </div>
            </div>
            <div class="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
              <p class="text-sm leading-6 text-gray-900">{{ $coba->instansi }}</p>
              <p class="mt-1 text-xs leading-5 text-gray-500">{{ $coba->status }} {{ $coba->jurusan }}</p>
            </div>
          </li>
          @endforeach
        </ul>
